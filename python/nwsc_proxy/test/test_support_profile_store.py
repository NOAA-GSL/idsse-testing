...
from python.nwsc_proxy.src.support_profile_store import SupportProfileStore, dt_parse

# constants
RAW_JSON_PATH = os.path.join(os.path.dirname(__file__), "..", "src", "supportprofiles")
...
def test_profile_store_loads_api_responses(store: SupportProfileStore, base_dir: str):
    ...
    for cache_id in store.profile_cache:
        ...
        filepath = os.path.join(base_dir, store.PROFILE_DIR, f"{cache_id}.json")
...
def test_save_adds_to_profiles(store: SupportProfileStore, base_dir: str):
    ...
    assert os.path.exists(os.path.join(base_dir, store.PROFILE_DIR, f"{new_profile_id}.json"))
...
def test_delete_profile(store: SupportProfileStore, base_dir: str):
    ...
    assert not os.path.exists(os.path.join(base_dir, store.PROFILE_DIR, f"{profile_id}.json"))
